...
model.MC.LS.FPID.radialWidth = .015; % [cm] Radial focal plane 1/e^2 radius if top-hat or Gaussian or half-width of the full distribution if custom
...
model.MC.LS.AID.radialWidth = 15/180*pi; % [rad] Radial angular 1/e^2 half-angle if top-hat or Gaussian or half-angle of the full distribution if custom. For a diffraction limited Gaussian beam, this should be set to model.MC.wavelength*1e-9/(pi*model.MC.LS.FPID.radialWidth*1e-2))
model.MC.LS.xFocus       = 0; % [cm] x position of focus
model.MC.LS.yFocus       = 0; % [cm] y position of focus
model.MC.LS.zFocus       = 0.03; % [cm] z position of focus
model.MC.LS.theta        = pi/6; % [rad] Polar angle of beam center axis
model.MC.LS.phi          = -pi/2; % [rad] Azimuthal angle of beam center axis
