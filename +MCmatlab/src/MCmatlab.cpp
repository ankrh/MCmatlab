...
void threadInitAndLoop(struct source *B_global, struct geometry *G_global,
          struct detectors *Dets_global, struct paths *Pa, struct outputs *O_global, struct depositionCriteria *DC_global, long nM, size_t size_smallArrays,
          ...
          bool *abortingPtr, bool silentMode, struct debug *Debug) {
  struct photon P_var;
  ...
  __shared__ struct detectors Dets_var;
  ...
  struct detectors *Dets= Dets_global;
  ...
  while(pctProgressThisWavelength < 100 && (requestCollectedPhotons? O->nPhotonsCollected: O->nPhotons) + THREADNUM < nPhotonsRequested && !*abortingPtr) { // "+ THREADNUM" ensures that we avoid race conditions that might launch more than nPhotonsRequested photons
  #endif
    launchPhoton(P,B,G,Pa,DC,abortingPtr,Debug);
    if(P->alive) getNewVoxelProperties(P,G,Debug);
    ...
    while(P->alive) { // keep doing scattering events
      while(P->alive && P->stepLeft>0) { // keep propagating
        propagatePhoton(P,G,O,DC,Pa,Debug);
        if(!P->sameVoxel) {
          checkEscape(P,Pa,G,Dets,O,DC,&O_global->nPhotonsCollected); // photon may die here
          if(P->alive) getNewVoxelProperties(P,G,Debug);
        }
      }
      if(P->alive) checkRoulette(P); // photon may die here
      if(P->alive) scatterPhoton(P,G,Pa,DC,Debug);
    }
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, mxArray const *prhs[]) {
  struct debug Debug_var = {{0.0,0.0,0.0},{0,0,0}};
  struct debug *Debug = &Debug_var;
  ...
  mxArray *MatlabDets = mxGetPropertyShared(MatlabMC,0,"Dets");
  struct detectors Dets_var;
  struct detectors *Dets = &Dets_var;
  ...
  for(idx=0;idx<Dets_var.nDetectors;idx++) {
    ...
    Det->r[2] = (FLOATORDBL)*mxGetPr(mxGetPropertyShared(MatlabDets,idx,"z"));
    Det->theta = (FLOATORDBL)*mxGetPr(mxGetPropertyShared(MatlabDets,idx,"theta"));
    Det->phi   = (FLOATORDBL)*mxGetPr(mxGetPropertyShared(MatlabDets,idx,"phi"));
  ...
  struct MATLABoutputs O_MATLAB_var = {
    ...
    Dets->nDetectors? (float **)malloc(Dets->nDetectors*sizeof(float *)): NULL,
  ...
  //   if(!silentMode) printf("Using %s with CUDA compute capability %d.%d,\n    launching %d blocks, each with %d threads,\n    using %d/%d bytes of shared memory per block\n",CDP.name,CDP.major,CDP.minor,blocks,threadsPerBlock,384+size_smallArrays,CDP.sharedMemPerBlock);
    ...
    struct source *B_dev;
    struct detectors *Dets_dev;
    struct paths *Pa_dev;
    struct outputs *O_dev;
    struct depositionCriteria *DC_dev;
    struct debug *Debug_dev;
    createDeviceStructs(G,&G_dev,B,&B_dev,Dets,&Dets_dev,Pa,&Pa_dev,O,&O_dev,DC,&DC_dev,nM,L,size_smallArrays,Debug,&Debug_dev);
    ...
    do {
      // Run kernel
      threadInitAndLoop<<<blocks, threadsPerBlock, size_smallArrays>>>(B_dev,G_dev,D_dev,Pa_dev,O_dev,DC_dev,nM,size_smallArrays,prevtime,clock/1000*min((long long)KERNELTIME,timeLeft),nPhotonsRequested_ThisWavelength,0,0,requestCollectedPhotons,NULL,false,D_dev);
    ...
    retrieveAndFreeDeviceStructs(G,G_dev,B,B_dev,Dets,Dets_dev,Pa,Pa_dev,O,O_dev,DC_dev,L,Debug,Debug_dev);
    ...
    #endif
    {
      threadInitAndLoop(B,G,Dets,Pa,O,DC,nM,0,simulationTimeStart,(long long)(simulationTimeRequested_ThisWavelength*60000000),nPhotonsRequested_ThisWavelength,iL,nL,requestCollectedPhotons,&aborting,silentMode,Debug);
    }
    #endif
    ...
    normalizeDepositionAndResetO(B,G,Dets,O,O_MATLAB,iL,B->power); // Convert data to relative fluence rate and save in O_MATLAB
  }
  ...
  *mxGetPr(output) = simulationTimeCumulative;
  ...
  free(O->NI_zpos);
  free(O->NI_zneg);
//   printf("\nDebug: %.18e %.18e %.18e %llu %llu %llu\n",Debug->dbls[0],Debug->dbls[1],Debug->dbls[2],Debug->ulls[0],Debug->ulls[1],Debug->ulls[2]);
}
