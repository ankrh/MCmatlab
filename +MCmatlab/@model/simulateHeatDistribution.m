...
if ~model.HS.silentMode && model.HS.makeMovie
  ...
  if model.HS.deferMovieWrite
    ...
    if contains(computer, 'MAC') % macOS operating system
      [~,~] = system([fileparts(caller(1).file) '/+MCmatlab/x264/x264_macOS -o "' fileparts(caller(1).file) '/' caller(1).name '_heatSimoutput.mkv" "' fileparts(caller(1).file) '/temp.avi"']);
    elseif contains(computer, 'WIN') % Windows operating system
      [~,~] = system([fileparts(caller(1).file) '\+MCmatlab\x264\x264_win64.exe -o "' fileparts(caller(1).file) '\' caller(1).name '_heatSimoutput.mkv" "' fileparts(caller(1).file) '\temp.avi"']);
    else % Linux operating system
      [~,~] = system([fileparts(caller(1).file) '/+MCmatlab/x264/x264_linux -o "' fileparts(caller(1).file) '/' caller(1).name '_heatSimoutput.mkv" "' fileparts(caller(1).file) '/temp.avi"']);
    end
