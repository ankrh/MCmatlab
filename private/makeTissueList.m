...
tissueList(j).mus = musp/(1-gg);
tissueList(j).g   = gg;
tissueList(j).VHC(j) =3391*1.109e-3;
tissueList(j).D(j) =1.109e-3;
tissueList(j).TC(j) =0.37e-2;
...
M = 0.03;
X = 2*[B*S B*(1-S) W M]';
tissueList(j).mua(j) = MU*X; %Here the hair is set to absorb twice as much as the epidermis
musp_hair = 9.6e10./nm.^3; % from Bashkatov 2002, for a dark hair
tissueList(j).g(j)   = 0.9;
tissueList(j).mus(j) = musp_hair/(1-g(j));
tissueList(j).VHC(j) = 1.53*1.3e-3; % Thermal data has been approximated using the data for horn, as horn and hair are both composed of keratin
tissueList(j).D(j)   = 1.3e-3;
tissueList(j).TC(j)  = 6.3e-3;
