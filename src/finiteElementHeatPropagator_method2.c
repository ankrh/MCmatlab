...
void mexFunction( int nlhs, mxArray *plhs[],int nrhs, mxArray const *prhs[] ) {
    ...
    #pragma omp parallel num_threads(1)//useAllCPUs || omp_get_num_procs() == 1? omp_get_num_procs(): omp_get_num_procs()-1)
    #endif
    {
		...
		double time_substep1 = 0, time_substep2 = 0, time_substep3 = 0;
...
// 			clock_gettime(CLOCK_MONOTONIC, &time_start);
            ...
            for(long iz=0; iz<nz; iz++) {
                for(long iy=0; iy<ny; iy++) {
					if(!(heatSinked && (iy == 0 || iy == ny-1 || iz == 0 || iz == nz-1))) { // If this x line is not on a heat sinked boundary
						for(long ix=0; ix<nx; ix++) { // Sweep up in x
							long i_xyz = ix + iy*nx + iz*nx*ny; // xyz index
							long i_zxy = iz + ix*nz + iy*nz*nx; // zxy index

							if(!(heatSinked && (ix == 0 || ix == nx-1))) { // If not constant-temperature boundary voxel
								// Explicit x
								...
								T_xline[ix] = T_xyz[i_xyz] + dt*((lightsOn? dTdt_abs[i_xyz]: 0) + dTdt_x/2 + dTdt_y + dTdt_z);
...
// 			time_substep1 += time_end.tv_sec  - time_start.tv_sec +
				...
				for(long ix=0; ix<nx; ix++) {
					if(!(heatSinked && (ix == 0 || ix == nx-1 || iz == 0 || iz == nz-1))) {
						for(long iy=0; iy<ny; iy++) { // Sweep up in y
							...
							b[iy] = 1;
							if(iy < ny-1) b[iy] += dt/2*c[nM*nM + M[i_xyz] + nM*M[i_xyz+nx]];
							if(iy > 0) {
								b[iy] +=  dt/2*c[nM*nM + M[i_xyz] + nM*M[i_xyz-nx]];
...
// 			clock_gettime(CLOCK_MONOTONIC, &time_end);
// 			time_substep2 += time_end.tv_sec  - time_start.tv_sec +
// 							(time_end.tv_nsec - time_start.tv_nsec)/1e9;
// 			
// 			clock_gettime(CLOCK_MONOTONIC, &time_start);
				...
				for(long ix=0; ix<nx; ix++) {
					if(!(heatSinked && (ix == 0 || ix == nx-1 || iy == 0 || iy == ny-1))) {
						for(long iz=0; iz<nz; iz++) { // Sweep up in z
							...
							b[iz] = 1;
							if(iz < nz-1) b[iz] += dt/2*c[2*nM*nM + M[i_xyz] + nM*M[i_xyz+nx*ny]];
							if(iz > 0) {
								b[iz] +=  dt/2*c[2*nM*nM + M[i_xyz] + nM*M[i_xyz-nx*ny]];
...
// 			clock_gettime(CLOCK_MONOTONIC, &time_end);
// 			time_substep3 += time_end.tv_sec  - time_start.tv_sec +
// 							(time_end.tv_nsec - time_start.tv_nsec)/1e9;
		...
		free(dT_y_array);
		free(T_xline);
// 		printf("1: %f, 2:%f, 3:%f \n",time_substep1,time_substep2,time_substep3);
    }
	free(T_yzx);
	free(T_zxy);
    return;
